...
void main() {
	// float d = 1.0 / 1024.0;
	// float fbon	= texture( uFbo, (vec2(1.0)-vUv) / 4.0 - vec2(0.0, d)).r;
	// float fbos	= texture( uFbo, (vec2(1.0)-vUv) / 4.0 + vec2(0.0, d)).r;
	// float fbow	= texture( uFbo, (vec2(1.0)-vUv) / 4.0 - vec2(d, 0.0)).r;
	// float fboe	= texture( uFbo, (vec2(1.0)-vUv) / 4.0 + vec2(d, 0.0)).r;
	...
	float tween = sin(uTime + vUv.s * 20.0 + vUv.t * 30.0);
	tween = pow(tween, 4.0);
	float roughnessMask = 0.5 - 0.4 * tween * texture( uFbo, fract(vec2(2.0) + (vec2(0.75)-vUv)/2.0)).r;
