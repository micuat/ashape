...
vec2 map( in vec3 pos )
{
    ...
    vec2 res = vec2( sdSphere(pos-vec3(-0.15,0.5+sin(iTime+a), 0), 0.1 ), 20.0 );
    ...
    if(depthOrg>0) {
        ...
        depth = 10*simplex3d(vec3(0.1, 0.1, iTime) + vec3(localFrag*5, idColor*20));// * pow(sin(iTime * 3.1415 / 4), 4);
        res = opU(res, 
        vec2(-(depthOrg-orgPos.z-1), 300 + 10 * depth)
        //;
        );
    }
