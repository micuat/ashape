...
var S110 = function (p) {
  ...
  this.drawPg = function(pg, t) {
    ...
    for(let i = 0; i < tentacles.length; i++) {
      let x = p.cos(t) * (300 - EasingFunctions.easeInQuint(1 - Math.abs(Math.sin(t * Math.PI * 0.2))) * (1-i)*100);
      let y = p.sin(t) * 300;
...
var s = function (p) {
  ...
  p.oscEvent = function (m) {
    ...
    if (path.length >= 3 && path[1] == "sc3p5" && path[2] == "spawn") {
      s110.spawn(p.createVector(m.get(0).floatValue(), 0));
    }
    else if (path.length >= 4 && path[1] == "sc3p5" && path[2] == "control") {
      s110[path[3]] = m.get(0).floatValue();
    }
  }
