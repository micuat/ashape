...
{
    float r = 0.2;
    vec2 res = vec2( sdPlane(     pos - vec3(0,-r,0)), 50.0 );
    // res = opU(res, vec2( sdSphere(pos - pos0, r ), 20.0 ));
    // res = opU(res, vec2( sdSphere(pos - pos1, r ), 20.0 ));
    // res = opU(res, vec2( sdSphere(pos - pos2, r ), 20.0 ));
    // res = opU(res, vec2( sdSphere(pos - pos3, r ), 20.0 ));
    res = opU(res, vec2(1.0/(1.0/(length(pos-pos0)) + 1.0/length(pos-pos1)) - 0.15, 20.0));
    res = opU(res, vec2(1.0/(1.0/(length(pos-pos2)) + 1.0/length(pos-pos3)) - 0.15, 20.0));
    return res;
}
