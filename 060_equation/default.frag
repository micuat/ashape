...
void main(void) {
  ...
  if(lightIntensity > 0.5) { 
	...
	if(iTime > 3.0) {
		visibility = mix(visibility, 36.0, (iTime-3.0)/1.0);
	}
	vec3 cback = vec3(1.0);
	vec2 nv = vec2(0.0);
	nv.x = simplex3d(vTexCoord.xyx*vec3(8.0,2.0, 2.0) + vec3(iTime * 1.0, iTime * 0.25, 0.0));
	nv.y = simplex3d(vTexCoord.xyx*vec3(20.0,10.0, 10.0) + vec3(iTime * -0.5, iTime * 0.5, 0.0));
	cback.gb = nv;

	nv.x *= -1;
	visibility = min(36.0, visibility + (nv.x+1.0) * 12.0);
	vec4 color0 = vec4(color * (cback + vec3(0.5) * min(visibility * 0.5 * 0.05556, lightIntensity)), vertColor.a); 
	vec4 color1 = vec4(color * min(visibility * 0.5 * 0.05556, lightIntensity), vertColor.a); 
	gl_FragColor = mix(color0, color1, saturate((visibility - 18.0) / 18.0));
  } else
