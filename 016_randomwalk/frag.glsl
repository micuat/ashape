...
float SE(in vec3 p){
	...
	float bump = sin(iTime * 0.5 + phi) * 0.25 + 0.5;	
	d = d - sr * (texture2D(texture, vec2(rho*2+3.1415, mod(phi*1+3.1415, 3.1415*2)) / 2 / 3.1415).r + 1);

	vec2 tc = floor(p.xz * 4) / 4 * 0.01 + vec2(0.5);
	return fBox(p, vec3(1000, 1000, texture2D(texture, tc) * 3).xzy);
}
...
void main() {
	...
	float spec = 2;
	float grey = pow(col.x, spec) * 20;

	float st = sin(iTime)*0.5+0.5;
	float ct = cos(iTime*0.4)*0.5+0.5;
	vec3 red = vec3(st,1-st,0);//vec3(1) - texture2D(texture, vec2(0, 0)).rgb;
	vec3 blue = vec3(0,1-ct,ct);//vec3(1) - texture2D(texture, vec2(1, 1)).rgb;
	if (grey > 0) {
		gl_FragColor = vec4(mix(red, blue, (ro+rd*t).y * 0.5) * grey, 1);
	}
	else {
