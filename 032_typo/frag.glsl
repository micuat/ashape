...
vec2 map( in vec3 pos )
{
    float h = 15.0;
    vec3 orgPos = pos;
    pos.z -= sin(iTime * 0.2) * 0.5- 0 + pos.x;//fract(iTime*0.1/6.0)*6.0;
    pos.x += iTime * 0.1;

    float ca = 0.35;
	float a = pos.x - (fract((pos.x + (0.5)) / ca) * ca - (0.5));
	float az = pos.z - (fract((pos.z + (0.5)) / 3) * 3 - (0.5));
	pos.x = (fract((pos.x + (0.5)+az*0.25) / ca) * ca - (0.5));
	pos.z = (fract((pos.z + (0.5)) / 7.5) * 7.5 - (0.5));

    vec2 res = vec2( sdCylinder(pos-vec3(-0.15,0, 0), vec2(0.075,h) ), a * 5 + 80 );

    float y = sin(iTime*1.5 + a*0.7)*0.4 + 0.5;

    vec4 stex = texture2D(u_depth, vertTexCoord.xy * 1);
    vec2 localFrag = stex.yz;
    float idColor = stex.a;
    float depthOrg = stex.r;
    if(depthOrg>0) {
        float depth = depthOrg;
        depth += -vertTexCoord.y*10.0+10.0*simplex3d(vec3(0.1, 0.1, iTime*2.0) + vec3(localFrag*vec2(4,2)+vec2(0,0), idColor*20));
        vec2 dRes = vec2(-(depth + 2.5 - orgPos.z-1), 150 * (depthOrg) + 10 * (+depth));
        res = opU(res, dRes);
        // res.x = mix(res.x, dRes.x, 0.1);
    }
...
{ 
    vec3 sky = vec3(0.5, 0.8, 0.5);//vec3(0.7, 0.9, 1.0)
