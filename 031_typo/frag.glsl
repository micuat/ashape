...
vec2 map( in vec3 pos )
{
    float h = 15.0;
    vec3 orgPos = pos;
    pos.z -= iTime * 0.2;//fract(iTime*0.1/6.0)*6.0;
    ...
    vec2 res = vec2( sdCylinder(pos-vec3(-0.15,0, 0), vec2(0.125,h) ), a * 5 + 100 );
    ...
    if(depthOrg>0) {
        ...
        depth = 1*simplex3d(vec3(0.1, 0.1, iTime*2.0) + vec3(localFrag*vec2(4,2)+vec2(0,0), idColor*20));
        vec2 dRes = vec2(-(depthOrg-orgPos.z-1), 150 * (depthOrg) + 10 * (+depth));
        res = opU(res, dRes);
