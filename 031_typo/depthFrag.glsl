...
void main() {
    vec4 vertColorMod = vertColor;
    float col = mix(nearColor.r, farColor.r, smoothstep(near, far, gl_FragCoord.z / gl_FragCoord.w));

    vertColorMod.r = texture2D(texture, vertTexCoord.xy * 1).a>0.5?1:0;

    // vertColorMod.r *= pow(simplex3d(vec3(1*vec2(15, 3) + vec2(+col*2, iTime*0.25), col)), 1.0);
    //gl_FragColor = vec4(col, fract(col*256), fract(col*256*256), 1.0);
    gl_FragColor = vec4(col, (vertTexCoord.xy*1)+vec2(0.0), vertColorMod.r);
}