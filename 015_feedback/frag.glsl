...
float SE(in vec3 p){
	...
	return d;//texture2D(texture, p.xy * 0.1) + p.z;
}
...
void main() {
	...
	vec3 red = vec3(sin(iTime)*0.5+0.5,0.2,0);//vec3(1) - texture2D(texture, vec2(0, 0)).rgb;
	vec3 blue = vec3(0,0.5,cos(iTime)*0.5+0.5);//vec3(1) - texture2D(texture, vec2(1, 1)).rgb;
	if (grey > 0) {
		gl_FragColor = vec4(mix(red, blue, (ro+rd*t).z * 0.5) * grey, 1);
	}
	else {
