...
var s = function (p) {
  ...
  p.setup = function () {
    ...
    textPg = p.createGraphics(p.width, p.height);
    shapePg = p.createGraphics(p.width, p.height, p.P3D);
  ...
  drawGeometry = function (pg, lights) {
    ...
    if(t % cycle > 7.0)
      ttween = 1.0 - Math.pow(p.map(t % cycle, 7.0, 8.0, 0.0, 1.0), 4.0);
      ...
      for (let j = -20; j <= 20; j++) {
        if (i == iSelected && isSphere(j)) {
          ...
          if (ribbonOrSS) {
            ...
            let rwidth = Math.sin((j + i) / 10.0 * Math.PI + t * 2.0);
            if(t % cycle > 7.0) {
              rwidth *= p.map(t % cycle, 7.0, 8.0, 1.0, 0.0);
            }
            pg.vertex(x, y, z - 5 * rwidth);
            pg.vertex(x, y, z + 5 * rwidth);
          }
          else {
  ...
  p.draw = function () {
    ...
    if (getCount() % (30 * cycle) == 0) {
      ...
      doWire = Math.random() > 0.5;

      colorScheme = p.random([
        function (pg, foreground, alpha) {
          if(foreground) {
            if(doWire) {
              pg.stroke(80, alpha == undefined ? 255 : alpha);
              pg.noFill();
            }
            else {
              pg.fill(80, alpha == undefined ? 255 : alpha);
              pg.noStroke();
            }
          }
          else
    ...
    if (t % cycle < 6) {
      ...
      textPg.translate(0, textPg.height / 2);
      textPg.fill(0, 128 * tweena);
      textPg.rect(0, -120, p.width, 240);

      textPg.translate(0, 25);

      // colorScheme(textPg, true, 255 * tweena);
      textPg.fill(255, 255 * tweena);

      textPg.textFont(font, 50);
      textPg.textAlign(p.LEFT, p.CENTER);
      textPg.text('remainders in movement', 50, -90);
      textPg.text('an accusation to form:', 50, -30);
      textPg.text('stay relevant, available', 50, 30);
      textPg.popMatrix();
    }
    textPg.endDraw();
    p.image(textPg, 0, 0);
