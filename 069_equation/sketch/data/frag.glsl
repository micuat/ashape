...
vec3 opTwist( vec3 p )
{
    float  c = cos(10.0*p.y+10.0+iTime);
    float  s = sin(10.0*p.y+10.0+iTime);
    mat2   m = mat2(c,-s,s,c);
...
vec2 map( in vec3 pos )
{
  ...
  float depth;
  float baseColor = 35.0 + sin(iTime * pi * 0.1) * 40.0 + 40.0;
  float col;
  if(obstex.r > 0.1) {
    depth = 0.01;
    col = 55.0;
    ...
    if(depth < 0.005) col = mix(20.0, col, depth * 200.0);
  }
